...
# --------------------------------------------------------------------------- #
# --format markdown (Component 15)
# --------------------------------------------------------------------------- #


def test_cli_diff_format_markdown_writes_to_stdout(capsys):
    rc = main(
        [
            "diff",
            _fx("era_evolution_v1.ttl"),
...
def test_cli_diff_format_markdown_writes_to_out_file_when_specified(tmp_path, capsys):
    ...
    assert "Generated by owlcompare" in written


def test_cli_diff_format_markdown_no_emoji_flag_uses_plain_tags(capsys):
    main(
        [
            "diff",
            _fx("era_evolution_v1.ttl"),
...
def test_cli_diff_format_markdown_heading_level_respected(capsys):
    main(
        [
            "diff",
            _fx("era_evolution_v1.ttl"),
...
def test_cli_diff_format_markdown_exit_code_matches_severity():
    # Markdown rendering must not change the breaking-change exit-code signal.
    assert (
        main(["diff", _fx("identical_a.ttl"), _fx("identical_b.ttl"), "--format", "markdown"]) == 0
    )
    assert (
        main(
            [
