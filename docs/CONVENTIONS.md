...
- Recommended setup on Windows with Smart App Control: install Python via winget (`winget install Python.Python.3.11`), then in the project run `uv python pin 3.11`. This avoids intermittent OS error 4551 blocks on uv-managed Python interpreters.
...
- On Windows with Smart App Control enabled, prefer `uv run python -m owlcompare ...` over the `uv run owlcompare ...` console script. The console-script wrapper (`owlcompare.exe`) is regenerated by `uv` and can be intermittently blocked by SAC with OS error 4551; the `python -m` form routes through the trusted `python.exe` and avoids the failure. See **DD-016**.
